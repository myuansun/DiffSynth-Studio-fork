...
def _get_ort_providers(device: str) -> tuple[list[str], Optional[list[dict]]]:
    import onnxruntime as ort

    available = set(ort.get_available_providers())
    if device != "cpu" and "CUDAExecutionProvider" in available:
        device_id = _parse_cuda_device_id(device)
        return ["CUDAExecutionProvider", "CPUExecutionProvider"], [{"device_id": device_id}, {}]
...
def _multiclass_nms(boxes: np.ndarray, scores: np.ndarray, nms_thr: float, score_thr: float) -> Optional[np.ndarray]:
    final_dets = []
...
def _preprocess_det(img: np.ndarray, input_size: tuple[int, int], swap: tuple[int, int, int] = (2, 0, 1)) -> tuple[np.ndarray, float]:
    if len(img.shape) == 3:
...
def _inference_detector(session, ori_img: np.ndarray) -> np.ndarray:
    input_shape = (640, 640)
    img, ratio = _preprocess_det(ori_img, input_shape)
    ort_inputs = {session.get_inputs()[0].name: img[None, :, :, :]}
    output = session.run(None, ort_inputs)
    predictions = _demo_postprocess(output[0], input_shape)[0]
    ...
    if dets is not None:
        ...
        is_score = final_scores > 0.3
        is_person = final_cls_inds == 0
        is_bbox = [bool(i and j) for (i, j) in zip(is_score, is_person)]
        final_boxes = final_boxes[is_bbox]
    else:
...
def _top_down_affine(input_size: tuple[int, int], bbox_scale: np.ndarray, bbox_center: np.ndarray, img: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    w, h = input_size
    warp_size = (int(w), int(h))

    bbox_scale = _fix_aspect_ratio(bbox_scale, aspect_ratio=w / h)
    warp_mat = _get_warp_matrix(bbox_center, bbox_scale, 0, output_size=(w, h))
...
def _decode(simcc_x: np.ndarray, simcc_y: np.ndarray, simcc_split_ratio: float) -> tuple[np.ndarray, np.ndarray]:
    keypoints, scores = _get_simcc_maximum(simcc_x, simcc_y)
    keypoints /= simcc_split_ratio
    return keypoints, scores


def _pose_preprocess(img: np.ndarray, out_bbox, input_size: tuple[int, int] = (192, 256)) -> tuple[list[np.ndarray], list[np.ndarray], list[np.ndarray]]:
    img_shape = img.shape[:2]
    out_img, out_center, out_scale = [], [], []
    if len(out_bbox) == 0:
        out_bbox = [[0, 0, img_shape[1], img_shape[0]]]
    for i in range(len(out_bbox)):
        x0, y0, x1, y1 = out_bbox[i][0], out_bbox[i][1], out_bbox[i][2], out_bbox[i][3]
...
def _inference_pose(session, out_bbox, ori_img: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    h, w = session.get_inputs()[0].shape[2:]
    model_input_size = (w, h)
    resized_imgs, centers, scales = _pose_preprocess(ori_img, out_bbox, model_input_size)
    all_key = []
    all_score = []
    for resized_img, center, scale in zip(resized_imgs, centers, scales):
        inp = resized_img.transpose(2, 0, 1)[None, ...].astype(np.float32)
        outputs = session.run(None, {session.get_inputs()[0].name: inp})
        simcc_x, simcc_y = outputs
        keypoints, scores = _decode(simcc_x, simcc_y, simcc_split_ratio=2.0)
...
class _Wholebody:
    def __init__(self, model_det: str, model_pose: str, *, device: str = "cpu"):
        import onnxruntime as ort

        providers, provider_options = _get_ort_providers(device)
        self.session_det = ort.InferenceSession(model_det, providers=providers, provider_options=provider_options)
        self.session_pose = ort.InferenceSession(model_pose, providers=providers, provider_options=provider_options)

    def __call__(self, ori_img: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        det_result = _inference_detector(self.session_det, ori_img)
        keypoints, scores = _inference_pose(self.session_pose, det_result, ori_img)
...
class _DWposeDetector:
    def __init__(self, model_det: str, model_pose: str, *, device: str = "cpu"):
        ...
        self.device = device
        self.pose_estimation: Optional[_Wholebody] = None
    ...
    def __call__(self, ori_img: np.ndarray) -> dict:
        if self.pose_estimation is None:
            self.pose_estimation = _Wholebody(self.model_det, self.model_pose, device=self.device)

        ori_img = ori_img.copy()
        h, w, _c = ori_img.shape
        candidate, score = self.pose_estimation(ori_img)
        ...
        for i in range(len(subset)):
            for j in range(len(subset[i])):
                subset[i][j] = int(18 * i + j) if subset[i][j] >= 0.3 else -1
        ...
        bodies = {"candidate": body, "subset": subset, "score": score[:, :18]}
        return {"bodies": bodies, "hands": hands, "hands_score": hands_score, "faces": faces, "faces_score": faces_score}
